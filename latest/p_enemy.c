...
/*            if (mo->type == MT_FATSO)
			...
			if (mo->flags2 & MF2_BOSS) // Tails 12-01-99
            { // Tails 12-01-99
                junk.tag = 666; // Tails 12-01-99
                for (z=0; z<MAXPLAYERS; z++)
					players[z].exiting = 2.8*TICRATE + 2;
...
void A_ParticleRise (mobj_t*   actor)
{
	if (actor->momz <= 0)
		actor->momz = JUMPGRAVITY*0.5;
	actor->momz += JUMPGRAVITY*0.1; // make bubbles rise!
}

void A_ParticleRise2 (mobj_t*   actor)
{
	if (actor->momz <= 0)
		actor->momz = JUMPGRAVITY*1;
	actor->momz += JUMPGRAVITY*0.2; // make bubbles rise!
}
