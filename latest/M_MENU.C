...
// Tails        S_StartSound(NULL,sfx_swtchx);
	...
	if (mycolor != myoldcolor)
		COM_BufAddText( va("%s %d", setupm_cvcolor->name, mycolor));
...
menuitem_t NewGameModeMenu[]=
{
    {IT_CALL | IT_STRING,"Campaign",M_ChooseMode, 100},
    {IT_CALL | IT_STRING,"Time Attack",M_ChooseMode,110}
};

menu_t  ModeDef =
{
    "M_NEWG",
    newgm_end,           // # of menu items
    &EpiDef,            // previous menu
    NewGameModeMenu,        // menuitem_t ->
    M_DrawNewGameMode,      // drawing routine ->
    48,63,              // x,y
    campaign            // lastOn
};
...
static consvar_t* ta_cvmap;

void M_ChooseMode(int choice)
{
    if (choice == campaign)
		M_SetupNextMenu(&NewDef);
	else if (choice == timeattack) {
		if (TimeAttackDef.lastOn != 0)
			TimeAttackDef.lastOn = 0;

		ta_cvmap = &cv_nozotimeattacklevel;
...
// srb2-nozomi time attack menu shenanigans

void M_DrawTimeAttack(void);
void M_StartTimeAttack(void);
void M_HandleTimeAttack(int choice);
...
{
    {IT_KEYHANDLER | IT_STRING, "Map", M_HandleTimeAttack, 10},
	{IT_CALL | IT_STRING,"Player Setup",M_SetupMultiPlayer, 160},
	{IT_CALL | IT_WHITESTRING,"Start", M_StartTimeAttack, 170}
};

menu_t  TimeAttackDef =
{
    "M_TATTAK",
    ta_end,           // # of menu items
    &ModeDef,            // previous menu
...
void M_StartTimeAttack(void)
{
	nozo_timeattack = true;
	G_DeferedInitNew(3, G_BuildMapName(epi+1,cv_nozotimeattacklevel.value));
    M_ClearMenus ();
}

void M_DrawTimeAttack(void)
{
#define LEVELNAMEMAX 128
	int minutes = 0;
	int seconds = 0;
	int centiseconds = 0;
	patch_t* lvlcard = W_CachePatchName("BLANKLVL",PU_CACHE);

	char levelname[LEVELNAMEMAX+1];

	if (strlen(mapheaders[cv_nozotimeattacklevel.value].name) > 0)
		if (mapheaders[cv_nozotimeattacklevel.value].act > 0)
			strncpy(levelname, va("%s Zone Act %d", mapheaders[cv_nozotimeattacklevel.value].name, mapheaders[cv_nozotimeattacklevel.value].act), LEVELNAMEMAX);
		else
			strncpy(levelname, va("%s Zone", mapheaders[cv_nozotimeattacklevel.value].name), LEVELNAMEMAX);
	else
		strncpy(levelname, G_BuildMapName(1,cv_nozotimeattacklevel.value), LEVELNAMEMAX);

	minutes = (mapheaders[cv_nozotimeattacklevel.value].ta_time/(60*TICRATE));
...
void M_HandleTimeAttack (int choice)
{
    ...
    switch( choice )
    {
      case KEY_DOWNARROW:
        S_StartSound(NULL,sfx_menu1); // Tails
        if (itemOn+1 >= TimeAttackDef.numitems)
            itemOn = 0;
        else itemOn++;
        break;
      ...
      case KEY_UPARROW:
        ...
        if (!itemOn)
            itemOn = TimeAttackDef.numitems-1;
        else itemOn--;
        break;
    ...
    if (mymap <1)
        mymap = 1;
    if (mymap >NUMMAPHEADERS-1)
        mymap = NUMMAPHEADERS-1;
    ...
    if (mymap != myoldmap) {
		if (mymap < myoldmap)
		{
			...
			for (i; i>-1; i--) {
				if(W_CheckNumForName(G_BuildMapName(gameepisode, i))!=-1)
				{
					mymap = i;
					break;
				}
		...
		else
		{
			...
			if (i > NUMMAPHEADERS-1)
				mymap = myoldmap;
		}

		COM_BufAddText ( va("%s \"%d\"",ta_cvmap->name ,mymap));
	}
...
{
    {IT_CALL | IT_STRING2,"Move Forward"    ,M_ChangeControl,gc_forward    },
    {IT_CALL | IT_STRING2,"Move Backward"   ,M_ChangeControl,gc_backward   },
	{IT_CALL | IT_STRING2,"Move Left"       ,M_ChangeControl,gc_strafeleft },
    {IT_CALL | IT_STRING2,"Move Right"      ,M_ChangeControl,gc_straferight},
    ...
    {IT_CALL | IT_STRING2,"Spin"            ,M_ChangeControl,gc_use        },
	{IT_CALL | IT_STRING2,"Ring Throw"      ,M_ChangeControl,gc_fire       },
...
void M_DrawVideoMode(void)
{
    ...
    else
    {
        ...
        sprintf(temp, "Current default is %s", VID_GetModeName(cv_scr_mode.value));
