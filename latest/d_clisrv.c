...
void CL_ConnectToServer()
{
    ...
    do {
        ...
        if( oldtic!=I_GetTime() )
        {
			...
			// start nozomi shenanigans
			D_ProcessEvents();
			D_PageTicker();
			D_PageDrawer("");
			// end nozomi shenanigans

            FiletxTicker();
            //CON_Drawer (); I don't need this Nozomi 03-10-2026
...
void Command_connect(void)
{
	...
	else
	{
		if (netgame)
		{
			CONS_Printf("You cannot connect to a server while in a netgame!\nClose the game first!\n");
			return;
		}
		else if (I_NetOpenSocket)
		{
			...
			CONS_Printf("There is no network driver or I screwed something up.\nLet me know @ admin [at] maril [dot] dev\n");
	}
...
void Got_KickCmd(char **p,int playernum)
{
    ...
    if( pnum==consoleplayer )
    {
		...
		{
			case KICK_MSG_GO_AWAY:
				   M_StartMessage("You have been kicked (Go away)\n\nPress ESC\n",NULL,true);
				   break;
			case KICK_MSG_CON_FAIL:
				   M_StartMessage("You have been kicked (Consistancy failure)\n\nPress ESC\n",NULL,true);
				   break;
			case KICK_MSG_TIMEOUT: // Tails 03-30-2001
				   M_StartMessage("You have been kicked (Connection timeout)\n\nPress ESC\n",NULL,true);
				   break; // Tails 03-30-2001
			case KICK_MSG_PLAYER_QUIT:
				   break;
		}
...
boolean SV_SpawnServer( void )
{
    ...
    if( gamestate == GS_DEMOSCREEN  || 
        gamestate == GS_NULL        || 
        gamestate == GS_WAITINGPLAYERS || gamestate == GS_NOZOMITITLE )
    {
