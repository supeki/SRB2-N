...
int M_MapNumber(char first, char second)
{
	...
	num = (100 // 100 is the beginning of the extended map numbers, 100 is A0
	+ ((int)tolower(first) - 'a') * 36         // get the first character's value, if it's A, assume it's 0, B is 36 (because each extended num can have 0-9, then A-Z 
	+ (isdigit(second) ? ((int)second - '0') : // if the second character is a number, add it to our total
		((int)tolower(second) - 'a') + 10));   // otherwise it's a character, so we do something similar to the above but instead of multiplying, 
											   // we add 10 to get past the number values
