...
//-----------------------------------------------------------------------------
//
// $Id: p_floor.c,v 1.7 2000/07/01 09:23:49 bpereira Exp $
//
...
void T_MoveFloor(floormove_t* floor)
{
    ...
    //SOM: Don't make noise if speed is MAXINT
    if (!(leveltime&7) && floor->speed < MAXINT)
    ...
    if (res == pastdest)
    {
        ...
        //SOM: Only make stoping noise if speed is less than MAXINT
        if(floor->speed < MAXINT)
          S_StartSound((mobj_t *)&floor->sector->soundorg,
                       sfx_pstop);
    }
