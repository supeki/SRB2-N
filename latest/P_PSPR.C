...
//-----------------------------------------------------------------------------
//
// $Id: p_pspr.c,v 1.2 2000/02/27 00:42:10 hurdler Exp $
//
...
#define LOWERSPEED              FRACUNIT*24 // Tails 03-13-2001
#define RAISESPEED              FRACUNIT*24 // Tails 03-13-2001
...
boolean P_CheckAmmo (player_t* player)
{
    ...
    count = weaponinfo[player->readyweapon].ammopershoot;
    ...
    do
    {
        if (player->weaponowned[wp_plasma]
            && player->ammo[am_cell]>=weaponinfo[wp_plasma].ammopershoot
            && (gamemode != shareware) )
        {
            player->pendingweapon = wp_plasma;
        }
        else if (player->weaponowned[wp_supershotgun]
                 && player->ammo[am_shell]>=weaponinfo[wp_supershotgun].ammopershoot
                 && (gamemode == commercial) )
        {
            player->pendingweapon = wp_supershotgun;
        }
        else if (player->weaponowned[wp_chaingun]
                 && player->ammo[am_clip]>=weaponinfo[wp_chaingun].ammopershoot)
        {
            player->pendingweapon = wp_chaingun;
        }
        else if (player->weaponowned[wp_shotgun]
                 && player->ammo[am_shell]>=weaponinfo[wp_shotgun].ammopershoot)
        {
            player->pendingweapon = wp_shotgun;
        }
        else if (player->ammo[am_clip]>=weaponinfo[wp_pistol].ammopershoot)
        {
        ...
        else if (player->weaponowned[wp_chainsaw])
        {
            player->pendingweapon = wp_chainsaw;
                 ...
                 && player->ammo[am_misl]>=weaponinfo[wp_missile].ammopershoot)
        {
            player->pendingweapon = wp_missile;
        }
        else if (player->weaponowned[wp_bfg]
                 && player->ammo[am_cell]>=weaponinfo[wp_bfg].ammopershoot
                 && (gamemode != shareware) )
        {
...
//    }
    ...
    if (player->cmd.buttons & BT_ATTACK) // Tails 12-03-2000
    {
		player->bustercount++; // Tails 12-12-2001
		if(((cv_gametype.value == 1 || cv_gametype.value == 4) && player->mo->health > 1 && !player->attackdown) || (cv_gametype.value == 3 && player->mo->health > 1 && !player->attackdown && player->tagit))
		{
			player->attackdown = true;
		player->mo->health--;
		player->health--;
		P_SpawnPlayerMissile (player->mo, MT_REDRING, player); // Tails 03-13-2001
			return;
		}
		else
        if ( player->mo->health > 1 && !player->attackdown
             && (!cv_gametype.value==1) && (player->charability == 3 || player->snowbuster)) // Megaman
        {
		P_SpawnPlayerMissile (player->mo, MT_REDRING, player);
		player->mo->health--;
		player->health--;
		player->attackdown = true;
            return;
	...
	{
		P_SpawnPlayerMissile (player->mo, MT_SNOWBALL, player); // Tails 12-12-2001
  ...
  pspdef_t*     psp )
{
    ...
    S_StartSound (player->mo, sfx_wtrdng);
...
//    P_SetMobjState (player->mo, S_PLAY_ATK2); // nono Tails 12-03-99
    ...
    //added:18-02-98: if NO AUTOAIM, or if autoaim didnt find something
    if (!(player->autoaim_toggle && cv_allowautoaim.value)
                             || (!linetarget && demoversion>111))
    {
        if(demoversion>=128)
            bulletslope = AIMINGTOSLOPE(player->aiming);
        else
            bulletslope = (player->aiming<<FRACBITS)/160;
    }
