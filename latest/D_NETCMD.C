...
consvar_t cv_playername           = {"name"                ,"sonic"    ,CV_CALL | CV_NOINIT | CV_SAVE,NULL,SendNameAndColor};
consvar_t cv_playercolor          = {"color"               ,"7"        ,CV_CALL | CV_NOINIT | CV_SAVE,Color_cons_t,SendNameAndColor};
consvar_t cv_playerautobrake      = {"autobrake"           ,"1"        ,CV_CALL | CV_NOINIT | CV_SAVE,CV_OnOff,SendNameAndColor};
// player's skin, saved for commodity, when using a favorite skins wad..
consvar_t cv_skin                 = {"skin"                ,DEFAULTSKIN,CV_CALL | CV_NOINIT | CV_SAVE,NULL /*skin_cons_t*/,SendNameAndColor};
...
consvar_t cv_playername2          = {"name2"               ,"sonic"    ,CV_CALL | CV_NOINIT | CV_SAVE,NULL,SendNameAndColor2};
consvar_t cv_playercolor2         = {"color2"              ,"7"        ,CV_CALL | CV_NOINIT | CV_SAVE,Color_cons_t,SendNameAndColor2};
consvar_t cv_playerautobrake2      = {"autobrake2"           ,"1"        ,CV_CALL | CV_NOINIT | CV_SAVE,CV_OnOff,SendNameAndColor2};
consvar_t cv_skin2                = {"skin2"               ,DEFAULTSKIN,CV_CALL | CV_NOINIT | CV_SAVE,NULL /*skin_cons_t*/,SendNameAndColor2};
...
//    COM_AddCommand ("turbo", Command_Turbo_f);     // turbo speed
  ...
  //
	...
	CV_RegisterVar (&cv_playerautobrake); // Autobrake! Nozomi 03-19-2026
	...
	CV_RegisterVar (&cv_playerautobrake2); // Autobrake! Nozomi 03-19-2026
	...
	CV_RegisterVar (&cv_nozotimeattacklevel);
...
//    CV_RegisterVar (&cv_allowturbo);
	...
	CV_RegisterVar (&cv_scr_mode);

    // add cheat commands, I'm bored of deh patches renaming the idclev ! :-)
    COM_AddCommand ("noclip", Command_CheatNoClip_f);
    COM_AddCommand ("god", Command_CheatGod_f);
    COM_AddCommand ("gimme", Command_CheatGimme_f);
	COM_AddCommand ("music", Command_Music_f);
	COM_AddCommand ("tunes", Command_Music_f); // alias for music!
	COM_AddCommand ("localtime", Command_Time_f); // test command for local time!
...
void SendNameAndColor(void)
{
	...
	if (!cv_playercolor.value) {
		...
		CV_SetValue(&cv_playercolor, i);
	}

	WRITEBYTE(p,cv_playercolor.value);
	WRITEBYTE(p,cv_playerautobrake.value);
    ...
    *(p-1) = 0; // finish the string;
	...
	if(cv_gametype.value == 4)
	{
		if(players[consoleplayer].ctfteam == 1 && cv_playercolor.value != SKINCOLOR_RED)
		{
...
void SendNameAndColor2(void)
{
	...
	WRITEBYTE(p,cv_playerautobrake2.value);
...
#define MAPNAME &buf[2]
	...
	if (strlen(mapname) < 3) {
		int mapnumber = M_MapNumber(mapname[0], mapname[1]);
		strcpy(MAPNAME, G_BuildMapName(1, mapnumber));
	}
...
void Command_Version_f (void)
{
    CONS_Printf ("Sonic Robo Blast 2 \"Nozomi\" %s", VERSIONSTRING);
...
void GameType_OnChange(void) // Tails 03-13-2001
{
    if(server && (multiplayer || netgame))
    {
		...
		if (cv_itemrespawntime.value < 5 && cv_gametype.value > 0)
			CV_SetValue(&cv_itemrespawntime,30);

		if(cv_gametype.value == 4)
		{
			COM_BufAddText ("timelimit 10\"");
		}
		else if(cv_gametype.value == 1 || cv_gametype.value == 3)
		{
