...
boolean PIT_CheckThing (mobj_t* thing)
{
   ...
   // check for special pickup
	...
	if(tmthing->flags & MF_SPECIAL || tmthing->flags & MF_ENEMY)
	{
		solid = tmthing->flags & MF_SOLID;
		if(thing->player)
			P_TouchSpecialThing(tmthing, thing); // can remove thing
		return !solid;
	}

// Push Gargoyles Tails 10-03-2001
// Push any Pushable Nozomi 03-09-2026
if (tmthing->player && thing->flags2 & MF2_PUSHABLE)
{
...
boolean PTR_AimTraverse (intercept_t* in)
{
    ...
    if ((!(th->flags&MF_SHOOTABLE) && !(th->flags&MF_ENEMY) && !(th->flags2&MF2_SPRING)) || (th->type == MT_DETON))
