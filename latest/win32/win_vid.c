...
void I_ShutdownGraphics (void)
{
    ...
    {
        // Hurdler: swap des deux lignes comme �a on close
        //          l'environnement OpenGL/glide apr�s avoir 
        //          vid� la cache ce qui est bcp plus propre
...
void I_FinishUpdate (void)
{
    ...
    if ( bDIBMode )
    {
        ...
        SetDIBitsToDevice (hDCMain,
                           0, 0, vid.width*vid.bpp, vid.height*vid.bpp,
                           0, 0, 0, vid.height*vid.bpp,
...
{
    return numvidmodes;   //faB: dont accept the windowed mode 0
}
...
static void WindowMode_Init(void)
{
	...
	for (i=0; i<NUMSPECIALMODES; i++) {
		specialmodes[i].pnext = &specialmodes[i+1];
	}

	specialmodes[NUMSPECIALMODES-1].pnext = NULL;
...
vmode_t* VID_GetModePtr(int modenum)
{
	...
	while(modenum--)
	{
		...
		if(!pv) {
			pv = oldpv;
			CONS_Printf("VID_error: Mode %d not available\n", modenum);
			break;
		}
	}
	return pv;
...
int VID_SetMode (int modenum)  //, unsigned char *palette)
{
	...
	if (modenum == MODE_CUSTOM) {
		...
		vid.rowbytes = cv_scr_width.value;
...
static  void VID_Command_Mode_f (void)
{
    ...
    if (modenum >= VID_NumModes()) //faB: dont accept the windowed mode 0
        CONS_Printf ("No such video mode\n");
    else {
        // request vid mode change
        setmodeneeded = modenum;
		CV_SetValue(&cv_scr_mode, modenum);
	}
