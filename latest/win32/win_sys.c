...
#define USE_WINDOWTITLE
void I_SetWindowTitle(char *WNDTTL) {
#ifdef USE_WINDOWTITLE
	if (!window_title)
		return;
	SetWindowTextA(hWndMain, WNDTTL);
#endif
}
...
localtime_t I_GetLocalTime(void) {
	...
	nozo_localtime.month = lt.wMonth;
	nozo_localtime.day = lt.wDay;
	nozo_localtime.year = lt.wYear;
	nozo_localtime.hour = lt.wHour;
	nozo_localtime.minute = lt.wMinute;
	nozo_localtime.second = lt.wSecond;
#endif

	return nozo_localtime; // even if USE_LOCALTIME is not defined, it should still return a struct full of 0s Nozomi 03-12-2026
}
