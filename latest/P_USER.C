...
// Insert same code for sidemove here Tails
	...
	else // Non-analog Sideways Movement Nozomi 03-18-2026
	{
		if(player->climbing)
			...
			// Swapped this out with the logic that used to be below! Nozomi 03-18-2026
...
// match their velocity.
		if(onground) // Only if on the ground.
		{
			if(player->walking)
			{
				if(player->speed > normalspeed/4)
					states[player->mo->state->nextstate].tics = 2;
				else if(player->speed > normalspeed/6)
			...
			{
				if(player->speed > normalspeed/4*5)
		...
		if(player->spinning)
			if(player->speed > normalspeed/5*2)
				states[player->mo->state->nextstate].tics = 1;
			else
				states[player->mo->state->nextstate].tics = 2;
...
// If so, begin the level end process. Tails 12-15-2000
		if(player->mo->subsector->sector->special == 982 && !player->exiting)
		{
			...
			if (nozo_timeattack && mapheaders[gamemap].ta_time > leveltime) {
				strncpy(mapheaders[gamemap].ta_name, cv_playername.string, 32);
				mapheaders[gamemap].ta_time = leveltime;
			}
		}
...
//////////////////////////
		...
		if(!(mapheaders[gamemap].special || cv_gametype.value == 1 || cv_gametype.value == 3
...
////////////////////////////
		...
		if(cmd->buttons &BT_USE && !player->exiting && !(player->mo->state == &states[S_PLAY_PAIN] && player->powers[pw_invisibility])) // subsequent revs
		{
			...
			else if(player->mfstartdash)
			{
				...
				if (leveltime & 1)
				{
					...
					{
						mobj_t* thok;
						thok = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_THOK);
		...
		// Relocated. Nozomi 03-15-2026
		if(player->mfspinning && player->speed > (normalspeed/2)+3 && !player->mfjumped)
		{
			{
				mobj_t* thok;
				thok = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z, MT_THOK);
...
////////////////////////////
		...
		if(cv_homing.value)
		{
			...
			if((!player->mo->tracer || (player->mo->tracer && player->mo->tracer->health <= 0)) && player->homing)
			{
...
///////////////////////
	...
	if(mapheaders[gamemap].special)
	{
		...
		if(player->sstimer > 1) // As long as time isn't up...
		{
			...
			if(multiplayer || netgame)
			{
				for(i=0; i<MAXPLAYERS; i++)
				{
					if(playeringame[i])
						if (mapheaders[gamemap].special == 2)
							ssrings += (players[i].sp_score) + (players[i].mo->health-1)*100;
						else
				...
				if (mapheaders[gamemap].special == 2)
					ssrings = player->sp_score + (player->mo->health-1)*100;
				else
	...
	// Removed xmasmode code Nozomi 03-18-2026

	if(!(player->powers[pw_invisibility] & 1))
		player->mo->flags &= ~MF_SHADOW;

	// What a strange implementation Nozomi 03-18-2026
	if(leveltime == 2)
	{
	...
	if (((player->speed + abs(player->mo->momz/FRACUNIT)) > normalspeed/3*2 && player->powers[pw_strength]) || player->homing) {
		...
		ghost->frame = player->mo->frame | (tr_transmed<<FF_TRANSSHIFT);
