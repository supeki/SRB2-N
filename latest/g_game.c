...
boolean G_Responder (event_t* ev)
{
    ...
    // any other key pops up menu if in demos (OR TITLE SCREEN Nozomi)
    if (gameaction == ga_nothing && !singledemo &&
        (demoplayback || gamestate == GS_DEMOSCREEN || gamestate == GS_NOZOMITITLE) )
    {
    ...
    switch (ev->type)
    {
      case ev_keydown:
		...
		if (ev->data1 == KEY_ESCAPE && gamestate == GS_WAITINGPLAYERS && netgame && (!server))
        {
...
void G_DoCompleted (void)
{
    ...
    wminfo.next = mapheaders[gamemap].next-1;
			...
			if(playeringame[i])
			{
				if(players[i].token)
				{
					if(!players[i].emerald1)
						...
						wminfo.next = SSSTAGE1; // Special Stage 2
					else if(!players[i].emerald3)
						wminfo.next = SSSTAGE1+1; // Special Stage 3
					else if(!players[i].emerald4)
						wminfo.next = SSSTAGE1+2; // Special Stage 4
					else if(!players[i].emerald5)
						wminfo.next = SSSTAGE1+3; // Special Stage 5
					else if(!players[i].emerald6)
						wminfo.next = SSSTAGE1+4; // Special Stage 6
					else if(!players[i].emerald7)
						wminfo.next = SSSTAGE1+5; // Special Stage 7
	...
	if((mapheaders[gamemap].special) && !gottoken)
	{
		wminfo.next = mapheaders[players[0].lastmap].next-1; // Exiting from a special stage? Go back to the game. Tails 08-11-2001
	}

	if(!(mapheaders[gamemap].special)) // Remember last map
	{
	for(i=0; i<MAXPLAYERS; i++)
		if(playeringame[i])
