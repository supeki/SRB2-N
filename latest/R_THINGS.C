...
static void R_InitMapHeaders(void);
                              ...
                              int                   x2 )
{
    ...
    else if ((vis->mobjflags & MF_TRANSLATION) && vis->transmap) // Color mapping
	{
		colfunc = transtransfunc;
		dc_transmap = vis->transmap;
		dc_translation = translationtables - 256 + ((int)vis->mobj->color<<8);
	}
    ...
    else if (vis->mobjflags & MF_TRANSLATION && vis->mobj->color)
    {
...
static void R_ProjectSprite (mobj_t* thing)
{
	...
	if (flip)
		offset = spriteoffset[lump] - spritewidth[lump];
	else
		offset = -spriteoffset[lump];

	offset = FixedMul(offset, skinscale);
	offset2 = FixedMul(spritewidth[lump], skinscale);
    ...
    gzt = thing->z + FixedMul(spritetopoffset[lump], skinscale);
    ...
    for(i=0;i<numskins;i++)
    {
        if (stricmp(skins[i].name,skinname)==0)
        {
            ...
            if (players[playernum].mo)
				...
				players[playernum].charability = skins[i].ability; // Tails
...
//            CONS_Error("ga");
            ...
            {
                strncpy (skins[numskins].face, value, 9);
...
// start character type identification Tails 03-01-2000
            ...
            {
                skins[numskins].ability = atoi(value);
            }
            else
...
void R_InitMapHeaders (void)
{
    ...
    {
		memset(&mapheaders[i], 0, sizeof(mapheader_t));
		//strncpy (mapheaders[i].name, "", LEVELNAMESIZE);
		mapheaders[i].act = 0;
		//strncpy (mapheaders[i].music, "D_RUNNIN", 8);
		//strncpy (mapheaders[i].sky, "SKY1", 8);
		mapheaders[i].next = i+1;
		mapheaders[i].special = 0;
		mapheaders[i].sp_time = 0;
		mapheaders[i].sp_rings = 0;
		mapheaders[i].hard_eggmobile = 0;
		strncpy (mapheaders[i].ta_name, "Unknown", 32);
		mapheaders[i].ta_time = 20999; // 9:59.99 ?
    }
}

int W_CheckForMapHeaderMarkerInPwad (int wadid, int startlump)
{
    int         i;
    lumpinfo_t* lump_p;
    ...
    if (startlump < wadfiles[wadid]->numlumps)
    {
        ...
        for (i = startlump; i<wadfiles[wadid]->numlumps; i++,lump_p++)
        {
            if (strncmp(lump_p->name, "MAP_HEAD", 8) == 0)
            {
                return ((wadid<<16)+i);
            }
        }
...
void R_AddMapHeader (int wadnum)
{
    ...
    //
    // search for all map_head markers in pwad
    //

    lastlump = 0;
    while ( (lumpnum=W_CheckForMapHeaderMarkerInPwad (wadnum, lastlump))!=-1 )
    {
        buf  = W_CacheLumpNum (lumpnum, PU_CACHE);
        size = W_LumpLength (lumpnum);
        ...
        buf2 = (char *) malloc (size+1);
        if(!buf2)
             I_Error("R_AddMapHeader: No more free memory\n");
        memcpy (buf2,buf,size);
        buf2[size] = '\0';
...
//            CONS_Error("ga");
            ...
            if (!value)
                I_Error ("R_AddMapHeader: syntax error in MAP_HEAD lump in WAD %s\n", wadfiles[wadnum]->filename);
			...
			if (!stricmp(token,"map"))
            {
                ...
                levelnum = M_MapNumber(tmp[0], tmp[1]);
            }
            ...
            if (!stricmp(token,"name"))
            {
				...
				// zone/level name
                strncpy (mapheaders[levelnum].name, value, LEVELNAMESIZE);

				for (i; i < LEVELNAMESIZE; i++) {
					if (mapheaders[levelnum].name[i] == '_')
						mapheaders[levelnum].name[i] = ' ';
				}
            }
            
            if (!stricmp(token,"act"))
            {
				// act number
                mapheaders[levelnum].act = atoi(value);
            }

			if (!stricmp(token,"titlecard"))
			{
				strncpy (mapheaders[levelnum].ttlcardlump, value, 8);
				strupr (mapheaders[levelnum].ttlcardlump);
			}

			if (!stricmp(token,"picname"))
			{
				// Pictures for Time Attack! Nozomi 03-13-2026
				strncpy (mapheaders[levelnum].picname, value, 8);
				strupr (mapheaders[levelnum].picname);
			}
            
            if (!stricmp(token,"music"))
            {
				// music lump name
                strncpy (mapheaders[levelnum].music, value, 8);
                strupr (mapheaders[levelnum].music);
            }

			if (!stricmp(token,"sky"))
            {
				// sky lump name
			...
			if (!stricmp(token,"next"))
			{
                ...
                mapheaders[levelnum].next = M_MapNumber(tmp[0], tmp[1]);
            }

			if (!stricmp(token,"special"))
            {
				// special stage!
                mapheaders[levelnum].special = atoi(value);
            }

			if (!stricmp(token,"sp_time"))
            {
				// special stage time to complete
                mapheaders[levelnum].sp_time = atoi(value);
            }

			if (!stricmp(token,"sp_rings"))
            {
				// special stage emerald requirements
                mapheaders[levelnum].sp_rings = atoi(value);
            }

			// add alternative name for sp requirement Nozomi 03-03-2026
			if (!stricmp(token,"sp_reqs"))
            {
				// special stage emerald requirements
                mapheaders[levelnum].sp_rings = atoi(value);
            }

			if (!stricmp(token,"hard_eggmobile"))
			{
				// eggmobile with spikeballs! Nozomi I Forgot The Date
				mapheaders[levelnum].hard_eggmobile = atoi(value);
			}

			if (!stricmp(token,"ta_name"))
			{
				// Time Attack! Nozomi 03-05-2026
				strncpy (mapheaders[levelnum].ta_name, value, 32);
			}

			if (!stricmp(token,"ta_time"))
			{
				// Time Attack! Nozomi 03-05-2026
				mapheaders[levelnum].ta_time = atoi(value);
			}
next_token:
            token = strtok (NULL,"\r\n= ");
        }
