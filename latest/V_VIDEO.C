...
//-----------------------------------------------------------------------------
//
// $Id: v_video.c,v 1.9 2000/04/27 17:43:19 hurdler Exp $
//
...
ULONG       pLocalPalette[256];     // local copy of the palette for V_GetColor()
...
// -------------+
// V_GetColor   : Get the ARGB value from a palette color index, using the
//              : last set palette.
// Returns      : ARGB 8888 value
// -------------+
ULONG V_GetColor( int color )
{
    return pLocalPalette[color&0xFF];
}
...
// Note         : we keep a copy of the palette so that we can get the RGB
...
void V_SetPalette( byte* palette )
{
    int     i;
    ULONG   rgb;
    byte*   usegamma = gammatable[scr_gamma%5];
    byte*   pal = palette;

    // keep a copy of the palette, for V_GetColor()
    for( i=0; i<256; i++ )
    {
        rgb = ( usegamma[*pal++] ) << 16;
        rgb |= ( usegamma[*pal++] ) << 8;
                         ...
                         byte*         colormap )
{
    ...
    if ( rendermode != render_soft) {
        HWR_DrawMappedPatch ((GlidePatch_t*)patch, x, y, colormap);
        return;
    }
#endif
...
void V_DrawScaledTranslationPatch ( int           x,
									int           y,
									int           scrn,    // hacked flags in it...
									patch_t*      patch,
									byte*		  colormap )
										  ...
										  byte*		  colormap )
{
    ...
    col = 0;
	pscale = FixedMul(dupx<<FRACBITS, scale);
	vscale = FixedMul(dupy<<FRACBITS, scale);
    colfrac  = FixedDiv (FRACUNIT, pscale);
    rowfrac  = FixedDiv (FRACUNIT, vscale);
    ...
    for ( ; (col>>FRACBITS)<patch->width ; col+=colfrac, desttop++)
    {
        column = (column_t *)((byte *)patch + LONG(patch->columnofs[col>>FRACBITS]));
        ...
        {
            source = (byte *)column + 3;
            dest   = desttop + (FixedMul(column->topdelta<<FRACBITS, vscale)>>FRACBITS)*vid.width;
            count  = FixedMul(column->length<<FRACBITS, vscale)>>FRACBITS;

            ofs = 0;
            while (count--)
            {
				if (dest >= screens[scrn&0xFF])
					*dest = *(colormap + (source[ofs>>FRACBITS]));
                dest += vid.width;
                ofs += rowfrac;
            }
