                    ...
                    int                   looping )
{
	...
	if (musicnum >= mus_runnin && strlen(mapheaders[gamemap].music) > 0) {
		music = &S_music[mus_runnin];
	} else {
		if ( (musicnum < mus_None) ||
			 (musicnum >= NUMMUSIC) )
		{
			CONS_Printf ("WARNING: Invalid music number %d, using %d instead\n", musicnum, mus_runnin);
			music = &S_music[mus_runnin];
		}
		else
	...
	if (musicnum >= mus_runnin && strlen(mapheaders[gamemap].music) > 0)
		strncpy(music_name, mapheaders[gamemap].music, 8);
	else
		strncpy(music_name, va("d_%s", music->name), 8);
	...
	music->lumpnum = W_GetNumForNameMusic(music_name);
	if (music->lumpnum == -1) {
		CONS_Printf("ERROR: Music lump %s doesn't exist!\n", music_name);
		return;
	}
...
void S_ChangeMusicName( char*                   musicname,
                    int                   looping )
{
    musicinfo_t*        music;
	char music_name[8];
	...
	music = &S_music[mus_runnin];
	strncpy(music_name, musicname, 8);
